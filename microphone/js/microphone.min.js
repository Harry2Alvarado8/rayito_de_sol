...
(function () {
    var a, b, c, d, e;
    navigator.getUserMedia = navigator.getUserMedia || navigator.webkitGetUserMedia || navigator.mozGetUserMedia || navigator.msGetUserMedia, window.AudioContext = window.AudioContext || window.webkitAudioContext || window.mozAudioContext || window.msAudioContext, function () {
        var a, b, c, d, e, f, g, h;
        for (e = window, h = ["ms", "moz", "webkit", "o"], f = 0, g = h.length; f < g && (d = h[f], !e.requestAnimationFrame); f++) e.requestAnimationFrame = e[d + "RequestAnimationFrame"], e.cancelAnimationFrame = e[d + "CancelAnimationFrame"] || e[d + "CancelRequestAnimationFrame"];
        ...
        c = 0, e.requestAnimationFrame = function (a) {
            var b;
    ...
    }(), d = ("undefined" != typeof localStorage && null !== localStorage ? localStorage.getItem : void 0) && localStorage.getItem("wit_debug") ? function () {
        return console.log.apply(console, arguments)
    } : function () {
    }, c = function (a, b) {
        return this.name = "WitError", this.message = a || "", this.infos = b, this
    }, c.prototype = Error.prototype, b = localStorage.getItem("wit_ws") || "wss://ws.wit.ai/speech_ws", a = function (a) {
        var b, c;
        return this.conn = null, this.ctx = new AudioContext, this.state = "disconnected", this.rec = !1, this.handleError = function (a) {
            var b, c;
            if (_.isFunction(c = this.onerror)) return b = _.isString(a) ? a : _.isString(a.message) ? a.message : "Something went wrong!", c.call(window, b, a)
        }, this.handleResult = function (a) {
            var b, c, d;
            if (_.isFunction(c = this.onresult)) return d = a.outcome.intent, b = a.outcome.entities, c.call(window, d, b, a)
            ...
            if (this.elem) return this.elem.classList.remove("active")
        }, this.mkactive = function () {
            if (this.elem) return this.elem.classList.add("active")
        }, this.mkthinking = function () {
            var a, b, d, e, f, g, h, i, j, k, l, m, n;
            if (this.thinking = !0, this.elem) return i = getComputedStyle(c), this.elem.classList.add("thinking"), l = parseInt(i.width, 10), f = parseInt(i.height, 10), b = "border-box" === i.boxSizing ? parseInt(i.borderTopWidth, 10) : 0, g = l / 2 - b - 5, a = 1e3, d = l / 2 - b, e = f / 2 - b - g, m = 0, j = 1, h = (null != (n = window.performance) ? n.now() : void 0) || new Date, k = function (c) {
                return function (i) {
                    var n, o, p, q;
        ...
        disconnected: {
            connect: function (a) {
                var c, e;
                return a || this.handleError("No token provided"), c = new WebSocket(b), c.onopen = function (b) {
                    return function (b) {
                        var e;
                        return d("connection opened", b), e = {
                            token: a,
                            bps: 16,
                            encoding: "signed-integer"
                        }, c.send(JSON.stringify(["auth", e]))
                    }
                ...
                }(this), this.conn = c, e = function (a) {
                    return function (b) {
                        var c, e, f;
                        return c = a.ctx, f = c.createMediaStreamSource(b), e = (c.createScriptProcessor || c.createJavascriptNode).call(c, 4096, 1, 1), e.onaudioprocess = function (b) {
                            var c, e, f, g, h, i, j, k;
                            if (a.rec) {
                                for (c = b.inputBuffer, e = c.getChannelData(0), h = e.length, g = new Int16Array(h), f = k = 0; 0 <= h ? k <= h : k >= h; f = 0 <= h ? ++k : --k) i = e[f], j = i < 0 ? 32768 * i : 32767 * i, g[f] = j;
                                return d("[audiobuffer] rate=" + c.sampleRate + ", samples=" + h + ", bytes=" + g.byteLength), a.conn.send(g)
                            }
                        }, f.connect(e), e.connect(c.destination), a.stream = b, a.proc = e, a.src = f, a.fsm("got_stream")
                    }
                }(this), navigator.getUserMedia({audio: !0}, e, this.handleError), "connecting"
            }
        }, connecting: {
            "auth-ok": function () {
            ...
            socket_closed: function () {
                ...
                return this.handleError(new c("Wit timed out", {code: "TIMEOUT"})), "ready"
            }, error: function (a) {
        ...
        var b, c, f, g;
        if (c = null != (g = e[this.state]) ? g[a] : void 0, b = Array.prototype.slice.call(arguments, 1), _.isFunction(c)) switch (f = c.apply(this, b), d("fsm: " + this.state + " + " + a + " -> " + f, b), this.state = f, "audiostart" !== f && "audioend" !== f && "ready" !== f && "connecting" !== f && "disconnected" !== f || _.isFunction(c = this["on" + f]) && c.call(window), f) {
            case"disconnected":
            case"ready":
        ...
        for (b in a) a[b], this.context[b] = a[b];
